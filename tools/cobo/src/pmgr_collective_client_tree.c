...
#define FLUX_CMB_MAX_STR 128
    ...
    char fencename[FLUX_CMB_MAX_STR];
    ...
    if ( snprintf (mydir,
                   FLUX_CMB_MAX_STR,
                   "lwj.%ld.cobo-bootstrap", 
                   cf_cxt->session) < 0 ) {
        pmgr_error("Could not copy session info"
            " @ file %s:%d",
            __FILE__, __LINE__
        );
    }
    ...
    if ( snprintf(keystr, FLUX_CMB_MAX_STR, 
                 "%s.%d", mydir, rank) < 0 ) {
        pmgr_error("Could not copy rank"
    ...
    if ( snprintf(valstr, FLUX_CMB_MAX_STR, 
                 "%s:%hd", inet_ntoa(ip), port) < 0 ) {
        pmgr_error("Could not copy ip:port into"
            " value buffer @ file %s:%d",
            __FILE__, __LINE__
        );
    }
